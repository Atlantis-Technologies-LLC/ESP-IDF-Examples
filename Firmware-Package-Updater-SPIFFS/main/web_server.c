...
#define WRITE_BLOCK_SIZE 4096 // SPIFFS Default: 4096 | LittleFS Default: 8192
...
static esp_err_t package_upload_handler(httpd_req_t *req) {
    ...
    // Extract firmware & SPIFFS sizes and offsets
    package_header_t pkg_header;
    memcpy(&pkg_header.firmware_size, write_buffer + 10, sizeof(uint32_t));
    memcpy(&pkg_header.spiffs_size, write_buffer + 10 + sizeof(uint32_t), sizeof(uint32_t));
    memcpy(&pkg_header.firmware_offset, write_buffer + 10 + (2 * sizeof(uint32_t)), sizeof(uint32_t));
    memcpy(&pkg_header.spiffs_offset, write_buffer + 10 + (3 * sizeof(uint32_t)), sizeof(uint32_t));

    ESP_LOGI(TAG, "Package contains: Firmware (%" PRIu32 " bytes), SPIFFS (%" PRIu32 " bytes)", 
         (uint32_t)pkg_header.firmware_size, (uint32_t)pkg_header.spiffs_size);

    remaining -= recv_len;

    ESP_LOGI(TAG, "Remaining after package header (%d bytes)",remaining);

    // **Unmount SPIFFS BEFORE updating firmware**
    ESP_LOGI(TAG, "Unmounting SPIFFS before firmware update...");
    ...
    // **Remount SPIFFS After updating firmware**
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to remount SPIFFS after firmware update: %s", esp_err_to_name(ret));
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "SPIFFS remounted successfully.");

    // **Handle SPIFFS File Updates**
    while (remaining > 0) {
        ...
        if (strlen(file_name) > 250) {  // Leave space for "/storage/"
            ESP_LOGE(TAG, "File path too long: %s", file_name);
            free(write_buffer);
            return ESP_FAIL;
        }

        snprintf(filepath, sizeof(filepath), "/storage/%s", file_name);
        ...
        while (file_written < file_size) {
            ...
            ESP_LOGI(TAG, "Writing SPIFFS file: %s, Remaining: %d bytes", filepath, file_remaining);
        }
...
static esp_err_t css_get_handler(httpd_req_t *req) {
    return file_get_handler(req, "/spiffs/styles.css", "text/css");
}

static esp_err_t firmware_update_html_get_handler(httpd_req_t *req) {
    return file_get_handler(req, "/spiffs/firmware-update.html", "text/html");
}

static esp_err_t firmware_update_js_get_handler(httpd_req_t *req) {
    return file_get_handler(req, "/spiffs/firmware-update.js", "application/javascript");
}
